...
export { OpenWindow, SystemWindowArgs, UseWindowManagerResult, WindowBounds, WindowManagerAction, WindowManagerActions, WindowManagerDispatch, WindowManagerProvider, WindowManagerState, WindowPayload, WindowState, initialWindowManagerState, useWindowManager, windowIdOf, windowManagerReducer } from './window-manager/index.cjs';
export { NotificationAction, NotificationInput, NotificationItem, NotificationSnapshot, clearAllNotifications, dismissNotification, getNotificationSnapshot, markAllNotificationsRead, markNotificationRead, notify, removeNotification, subscribeNotifications, useNotifications } from './notifications/index.cjs';
...
type ThemeAppearance = ResolvedAppearance | "auto";
/**
...
interface OsTheme {
     ...
     */
    appearances?: {
        light?: OsThemeAppearance;
        dark?: OsThemeAppearance;
    };
    /**
...
interface ColorFromPaletteField extends CustomizableBase {
    kind: "color-from-palette";
    /** Hex / CSS color strings the user picks from. */
    options: string[];
}
interface ImagePickField extends CustomizableBase {
    kind: "image-pick";
    options: Array<{
        src: string;
        label: string;
...
interface RangeField extends CustomizableBase {
    kind: "range";
    min: number;
    max: number;
    step: number;
...
interface SelectField extends CustomizableBase {
    kind: "select";
    options: Array<{
        value: string;
        label: string;
...
interface ToggleField extends CustomizableBase {
    kind: "toggle";
}
type CustomizableField = ColorFromPaletteField | ImagePickField | RangeField | SelectField | ToggleField;
/** User-chosen overrides keyed by the same dotted paths used by `customizable`. */
type SettingsPrefs = Record<string, unknown>;
 ...
 */
declare function setPath<T>(obj: T, path: string, value: unknown): T;
/**
 ...
 */
declare function applyPrefs(theme: OsTheme, prefs: SettingsPrefs): OsTheme;
/**
 ...
 * color scheme).
 */
declare function applyAppearance(theme: OsTheme, mode: ResolvedAppearance): OsTheme;

export { type App, type AppContentProps, type ColorFromPaletteField, type CustomizableField, type ImagePickField, type OsTheme, type OsThemeAppearance, type OsThemeBlur, type OsThemeChrome, type OsThemeElevation, type OsThemeMotion, type OsThemePalette, type OsThemeShape, type OsThemeWallpaper, type RangeField, type ResolvedAppearance, type SelectField, type SettingsPrefs, type StorageAdapter, type ThemeAppearance, type ToggleField, applyAppearance, applyPrefs, createLocalStorageAdapter, getPath, setPath };
